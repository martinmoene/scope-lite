...
table = (
    ...
    , ( 'CMakeLists.txt'
        , r'set\W+scope_lite_version\W+"([0-9]+\.[0-9]+\.[0-9]+)"\W+$'
        , 'set( scope_lite_version "{major}.{minor}.{patch}" )\n' )

#    , ( 'example/cmake-pkg/CMakeLists.txt'
#        , r'set\W+scope_lite_version\W+"([0-9]+\.[0-9]+(\.[0-9]+)?)"\W+$'
#        , 'set( scope_lite_version "{major}.{minor}" )\n' )
#
#    , ( 'script/install-xxx-pkg.py'
#        , r'\scope_lite_version\s+=\s+"([0-9]+\.[0-9]+\.[0-9]+)"\s*$'
#        , 'scope_lite_version = "{major}.{minor}.{patch}"\n' )
...
#        , r'\#define\s+scope_lite_MAJOR\s+[0-9]+\s*$'
#        , '#define scope_lite_MAJOR  {major}' )

    , ( 'include/nonstd/scope.hpp'
        , r'\#define\s+scope_lite_MINOR\s+[0-9]+\s*$'
        , '#define scope_lite_MINOR  {minor}' )

    , ( 'include/nonstd/scope.hpp'
        , r'\#define\s+scope_lite_PATCH\s+[0-9]+\s*$'
        , '#define scope_lite_PATCH  {patch}\n' )
)
